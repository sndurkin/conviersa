	  ...
	  m_savedMaxUsers(0)
{
	...
	m_pDelegate = new IrcChanTopicDelegate(m_pView);
...
void IrcChanListWindow::GiveFocus()
{
	if(m_pView)
		m_pView->setFocus();
}
...
// channels are added
void IrcChanListWindow::BeginPopulatingList()
{
...
void IrcChanListWindow::AddChannel(const QString &channel, const QString &numUsers, const QString &topic)
{
	if(m_populatingList)
	{
...
void IrcChanListWindow::EndPopulatingList()
{
...
void IrcChanListWindow::ClearList()
{
	m_pModel->clear();
...
void IrcChanListWindow::SetupControls()
{
	// controls layout
...
void IrcChanListWindow::HandleConnect()
{
	ClearList();
...
void IrcChanListWindow::HandleDisconnect()
{
...
void IrcChanListWindow::DownloadList()
{
...
void IrcChanListWindow::StopDownload()
{
	m_pSharedConn->Send("LIST STOP");
}

// joins the channel which is found with the given index
void IrcChanListWindow::JoinChannel(const QModelIndex &index)
{
...
void IrcChanListWindow::StartFilter()
{
...
void IrcChanListWindow::PerformSearchIteration()
{
	int i;
...
void IrcChanListWindow::StopFilter()
{
	m_pSearchTimer->stop();
	m_pSearchBar->ClearProgress();
...
void IrcChanListWindow::SaveList()
{
	if(m_pModel->rowCount() == 0)
		return;
